...
class MahlerNet:
	...
	def __init__(self, params):
		...
		if p["use_gpu"] and tf.test.is_gpu_available():
			p["gpu"] = True
			p["cell_versions"]["lstm"]["cudnn"] = tf.keras.layers.LSTM
			p["cell_versions"]["gru"]["cudnn"] = tf.keras.layers.GRU
	...
	def build(self):
		...
		def _inputs(o, p):
			...
			with tf.name_scope("inputs"):
				o.inp_enc_dropout = tf.keras.layers.Input(shape=(), name='inp_enc_dropout', dtype=tf.float32)
				o.ctx_enc_dropout = tf.keras.layers.Input(shape=(),name = "ctx_enc_dropout", dtype=tf.float32)
				o.vae_dropout = tf.compat.v1.placeholder(tf.float32, (), name = "vae_dropout")
				o.dec_dropout = tf.compat.v1.placeholder(tf.float32, (), name = "dec_dropout")
				o.dec_balstm_conv_dropout = tf.compat.v1.placeholder(tf.float32, (), name = "dec_balstm_conv_dropout")
				o.pitch_dropout = tf.compat.v1.placeholder(tf.float32, (), name = "pitch_dropout")
				o.offset_dropout = tf.compat.v1.placeholder(tf.float32, (), name = "offset_dropout")
				o.duration_dropout = tf.compat.v1.placeholder(tf.float32, (), name = "duration_dropout")
				o.instrument_dropout = tf.compat.v1.placeholder(tf.float32, (), name = "instrument_dropout")
				o.batch_sz = tf.compat.v1.placeholder(tf.int32, (), name = "batch_sz")
				o.global_step = tf.compat.v1.get_variable("global_step", (), dtype = tf.float32, trainable = False, initializer=tf.compat.v1.constant_initializer(0.0))
				# placeholders for inputs needed during generation
				o.instrument_temp = tf.compat.v1.constant(1.0, dtype = tf.float32, name = "instrument_temp")
				o.pitch_temp = tf.compat.v1.constant(1.0, dtype = tf.float32, name = "pitch_temp")
				o.offset_temp = tf.compat.v1.constant(1.0, dtype = tf.float32, name = "offset_temp")
				o.duration_temp = tf.compat.v1.constant(1.0, dtype = tf.float32, name = "duration_temp")
				o.sample_from_predictions = tf.compat.v1.constant(False, dtype = tf.bool, name = "sample_from_predictions")
				o.training = tf.compat.v1.placeholder(tf.bool, (), name = "training")
				o.durations = tf.compat.v1.placeholder(tf.float32, (1, 1, p["num_durations"]), name = "durations")
				o.last_time = tf.compat.v1.placeholder(tf.float32, (), name = "last_time")
				
				o.bar = tf.compat.v1.placeholder(tf.float32, (), name = "bar")
				o.beats = tf.compat.v1.placeholder(tf.float32, (1, 1, p["num_beats"]), name = "beats")
				o.inp_lengths = tf.compat.v1.placeholder(tf.int32, [None], name = "inp_lengths")
				...
				if p["dec"]["model"] == "balstm": # have to split pitch properties and the rest into different parts
					...
					x_a = []
				if p["model"]["ctx"]:
					o.ctx_s = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_special_tokens"]), name = "ctx_s")
					ctx = [o.ctx_s]
				if p["modelling_properties"]["offset"]["include"]:
					print("[MAHLERNET]: adding \"offset\" as a parameter to model")
					o.x_o = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_durations"]), name = "x_o")
					o.y_o_ = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_durations"]), name = "y_o_")
					...
					if p["dec"]["model"] == "balstm":
						...
						x += [o.x_o]
					if p["model"]["ctx"]:
						o.ctx_o = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_durations"]), name = "ctx_o")
						ctx += [o.ctx_o]
					y += [o.y_o_]
				if p["modelling_properties"]["beats"]["include"]:
					print("[MAHLERNET]: adding \"beats\" as a parameter to model")
					o.x_b = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_beats"]), name = "x_b")
					o.y_b_ = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_beats"]), name = "y_b_")
					if p["dec"]["model"] == "balstm":
						x_rest += [o.x_b]
					else:
						x += [o.x_b]
					if p["model"]["ctx"]:
						o.ctx_b = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_beats"]), name = "ctx_b")
						ctx += [o.ctx_b]
					y += [o.y_b_]
				if p["modelling_properties"]["duration"]["include"]:
					print("[MAHLERNET]: adding \"duration\" as a parameter to model")
					o.x_d = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_durations"]),name = "x_d")
					o.y_d_ = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_durations"]),name = "y_d_")
					if p["training_output"]["dists"]:
					...
					if p["dec"]["model"] == "balstm":
						...
						x += [o.x_d]
					if p["model"]["ctx"]:
						o.ctx_d = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_durations"]), name = "ctx_d")
						ctx += [o.ctx_d]	
					y += [o.y_d_]
				if p["modelling_properties"]["pitch"]["include"]:
					print("[MAHLERNET]: adding \"pitch\" as a parameter to model")
					o.x_p = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_pitches"]), name = "x_p")
					o.y_p_ = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_pitches"]), name = "y_p_")
					...
					if p["modelling_properties"]["active_pitches"]["include"]:
						...
						o.x_ap = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_pitches"]), name = "x_ap")
						if p["dec"]["model"] != "balstm":
							x_a += [o.x_ap]
					if p["model"]["ctx"]:
						o.ctx_p = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_pitches"]), name = "ctx_p")
						ctx += [o.ctx_p]							
					y += [o.y_p_]					
				if p["modelling_properties"]["instrument"]["include"]:
					print("[MAHLERNET]: adding \"instrument\" as a parameter to model")
					o.x_i = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_instruments"]),name = "x_i")
					o.y_i_ = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_instruments"]),name = "y_i_")
					...
					if p["modelling_properties"]["active_instruments"]["include"]:
						...
						o.x_ai = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_instruments"]),name = "x_ai")
						if p["dec"]["model"] == "balstm":
							x_rest += [o.x_ai]
						else:
							x_a += [o.x_ai]
					if p["model"]["ctx"]:
						o.ctx_i = tf.compat.v1.placeholder(tf.float32, (None, None, p["num_instruments"]), name = "ctx_i")
						ctx += [o.ctx_i]	
					y += [o.y_i_]
				if p["dec"]["model"] == "balstm":
					if len(x_rest) > 0:
				...
				if p["model"]["ctx"]:
					...
					o.ctx_lengths = tf.compat.v1.placeholder(tf.int32, [None], name = "ctx_lengths")
		...
		def _rnn_setup(batch_sz, cell_type, cell_version, versions, bidirectional, state_sz, num_layers, rnn_dropout, proj_dropout, init_state, prefix, training):
			...
			with tf.variable_scope(prefix + "_rnn_setup"):
				...
				'''
				cells = [tf.keras.layers.LSTMCell(units) for _ in range(num_layers)]

				if rnn_dropout is not None:
  					cells = [tf.keras.layers.Dropout(rate=1.0 - rnn_dropout, noise_shape=(None, 1))(cell) for cell in cells]

				cell = tf.keras.layers.StackedRNNCells(cells)

				#model = tf.keras.models.Sequential([tf.keras.layers.RNN(cell, return_sequences=True)])
			...
			with tf.variable_scope(prefix + "_rnn"):
				...
				if bidirectional is not None:
					...
					if bidirectional == "stack":
						...
						output, final_state_fw, final_state_bw = tf.keras.layers.StackedBidirectional(cell[0], cell[1],**kwargs)
						final_state=(final_state_fw,final_state_bw)
					else:
						print("[MAHLERNET]: adding a bidirectional default " + prefix + "_rnn ")
						kwargs = {"inputs": input, "initial_state_fw": init_fw, "initial_state_bw": init_bw,
								  "dtype": tf.float32, "sequence_length": seq_lengths}
						output, final_state = tf.keras.layers.Bidirectional(cell[0], **kwargs)
				else:
					...
					output, final_state = tf.keras.layers.RNN(cell[0], **kwargs)
		...
		def _seq2seq(o, cell, input, batch_sz, seq_lengths, cell_type, prefix, fn = None, aux = None):
			...
			with tf.variable_scope(prefix + "_seq2seq"):
				...
				if not p["dec"]["scheduled_sampling"]:
					helper = tf.compat.v1.nn.seq2seq.TrainingHelper(input, seq_lengths) 
				...
				# sample ids are False for outputs where no sampling took place, True otherwise
					...
					helper = tf.compat.v1.seq2seq.ScheduledOutputTrainingHelper(input, seq_lengths, prob, next_inputs_fn = fn, auxiliary_inputs = aux)
				
				kwargs = {"maximum_iterations": o.inp_lengths_max, "impute_finished": True}				
				decoder = tf.compat.v1.seq2seq.BasicDecoder(cell[0], helper, init, None)
				...
				'''
				decoder_output_tuple, decoder_final_state, sequences = tf.compat.v1.seq2seq.dynamic_decode(decoder, **kwargs)
					...
					if p["dec"]["init"] == "z": # set up the initial state as a projection from the latent vector
						...
						if p["dec"]["model"] == "balstm":
							...
							if p["dec"]["balstm"]["init"] == "diff": # use different projections of latent space for all rnn instances
								if p["dec"]["layer_strategy"] == "diff":
									...
									if p["vae"]["dropout"] > 0.0:
										 # (batch, num_p * state_sz)
										o.dec_init = [tf.keras.layers.Dropout(rate = o.vae_dropout)(init_layer) for init_layer in o.dec_init]
									if p["batch_norm"] and not p["batch_norm_before_act"]:
										# (batch, num_p * state_sz)
										o.dec_init = [tf.keras.layers.BatchNormalization()(init_layer, training = o.training) for init_layer in o.dec_init]
									o.dec_init = [tf.reshape(in_st, [o.balstm_batch_sz, p["dec"]["sz_state"]]) for in_st in o.dec_init]
								else: # same setup for every layer
									if p["batch_norm"] and p["batch_norm_before_act"]:
										o.dec_init = [p["act"](tf.layers.batch_normalization(tf.layers.Dense(p["dec"]["sz_state"] * p["num_pitches"], activation = p["act"])(o.z), training = o.training)) for _ in range(layer_states)] # (batch, num_p * state_sz)
									...
									if p["vae"]["dropout"] > 0.0:
										o.dec_init = [tf.keras.layers.Dropout(rate = o.vae_dropout)(init_layer) for init_layer in o.dec_init] # (batch, num_p * state_sz)
									if p["batch_norm"] and not p["batch_norm_before_act"]:
										o.dec_init = [tf.keras.layers.BatchNormalization()(init_layer, training = o.training) for init_layer in o.dec_init] # (batch, num_p * state_sz)
									o.dec_init = [tf.reshape(in_st, [o.balstm_batch_sz, p["dec"]["sz_state"]]) for in_st in o.dec_init]
								...
								if p["dec"]["layer_strategy"] == "diff": # different between layers however
									...
									if p["vae"]["dropout"] > 0.0:
										o.dec_init = [tf.keras.layers.Dropout(rate = o.vae_dropout)(init_layer)  for init_layer in o.dec_init] # (batch, sz_state)
									if p["batch_norm"] and not p["batch_norm_before_act"]:
										o.dec_init = [tf.keras.layers.BatchNormalization()(init_layer, training = o.training) for init_layer in o.dec_init] # (batch, sz_state)
									...
									if p["vae"]["dropout"] > 0.0:	
										o.dec_init_raw = tf.keras.layers.Dropout(o.vae_dropout)(o.dec_init_raw)
									if p["batch_norm"] and not p["batch_norm_before_act"]:
							...
							if p["dec"]["type"] == "lstm":
								...
								o.dec_init = [tf.keras.layers.LSTMStateTuple(o.dec_init[a], o.dec_init[a + 1]) for a in range(0, tot_states, 2)] # one per layer
							...
							if o.dec_init_raw is None: # either we are NOT using the BALSTM OR we are using a strategy with the BALSTM that makes it impossible to use a single projection of z as a feed during decoding
								...
								if p["dec"]["init"] != "z" or p["dec"]["model"] == "balstm" or (p["dec"]["model"] == "lstm" and p["dec"]["layer_strategy"] == "diff"): 
									...
									if p["vae"]["dropout"] > 0.0:
										...
										o.dec_init_raw = tf.keras.layers.Dropout(rate = o.vae_dropout)(o.dec_init_raw) # (batch, sz_state)
		...
		def _balstm_decoder(o, p, w):
			...
			with tf.variable_scope("balstm_decoder"):
				...
				if p["dec"]["balstm"]["conv_dropout"] > 0.0:
					...
					o.conv_output = tf.keras.layers.Dropout(rate = o.dec_balstm_conv_dropout)(o.conv_output)
				if p["batch_norm"] and not p["batch_norm_before_act"]:
				...
				for i in range(p["dec"]["num_layers"]):
					if p["dec"]["type"] == "lstm":
						...
						inits.append(tf.keras.layers.LSTMStateTuple(getattr(o, "dec_init_c_" + str(i)), getattr(o, "dec_init_h_" + str(i))))
					else:
			...
			with tf.variable_scope("lstm_decoder"):
				...
				for i in range(p["dec"]["num_layers"]):
					if p["dec"]["type"] == "lstm":
						setattr(o, "dec_init_h_" + str(i), init[0][i].h)
						setattr(o, "dec_init_c_" + str(i), init[0][i].c)
						inits += [tf.compat.v1.nn.rnn_cell.LSTMStateTuple(getattr(o, "dec_init_c_" + str(i)), getattr(o, "dec_init_h_" + str(i)))]
			...
			for num, layer in enumerate(layers):
				...
				if num < len(layers) - 1: # no dropout or post activation BN on last output layer
					if p["modelling_properties"][prefix]["dropout"] > 0.0: 
						...
						logits = tf.keras.layers.Dropout(rate = p["modelling_properties"][prefix]["dropout"])(logits)
				if p["batch_norm"] and not p["batch_norm_before_act"]:
					...
					for cat in ["offset", "duration", "pitch", "instrument"]:									  
						if p["modelling_properties"][cat]["include"]:
							...
							if cat_params["multiclass"]:
								...
								loss = tf.losses.binary_crossentropy(logits = logits, labels = targets, from_logits=True)
